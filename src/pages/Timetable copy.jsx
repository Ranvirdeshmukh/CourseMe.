...
const Timetable = () => {
  ...
  useEffect(() => {
    fetchCoursesFromCloudStorage();
  }, []);

  const fetchCoursesFromCloudStorage = async () => {
    try {
      const storage = new Storage({
        keyFilename: process.env.GOOGLE_AUTHENTICATION
      });

      const bucketName = process.env.CLOUD_STORAGE_BUCKET_NAME;
      const fileName = 'current_courses.json';

      const lockFileName = `${fileName}.lock`;
      const [lockFileExists] = await storage.bucket(bucketName).file(lockFileName).exists();

      if (lockFileExists) {
        // If lock file exists, wait for a short time and retry
        await new Promise(resolve => setTimeout(resolve, 1000));
        return fetchCoursesFromCloudStorage();
      }

      const [fileContent] = await storage.bucket(bucketName).file(fileName).download();
      const coursesData = JSON.parse(fileContent.toString());

      // Process the coursesData to match the format we need
      ...
      console.error('Error fetching courses from Cloud Storage:', error);
      setError(error);
      setLoading(false);
    }
  };
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedCourses.map((course, index) => {
                    const limit = parseInt(course.limit);
                    const enrolled = parseInt(course.enrolled);
                    const available = isNaN(limit) || isNaN(enrolled) ? 0 : Math.max(0, limit - enrolled);
                    const availabilityColor = getAvailabilityColor(available, limit);
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell sx={{ color: 'black', padding: '10px', textAlign: 'left' }}>{course.instructor}</TableCell>
                        <TableCell sx={{ color: 'black', padding: '10px', textAlign: 'left' }}>
                      {course.limit !== '&nbsp' && course.enrolled !== '&nbsp' ? (
                        <Tooltip title={`${available} out of ${limit} spots available`}>
                          <Box>
                            <LinearProgress
                              variant="determinate"
                              value={(enrolled / limit) * 100}
                              color={availabilityColor}
                              sx={{ height: 10, borderRadius: 5 }}
                            />
                            <Typography variant="body2" sx={{ mt: 1 }}>
                              {available} / {limit} {course.ip && '*'}
                            </Typography>
                          </Box>
                        </Tooltip>
                      ) : (
