...
class CachedCourseAnalysis:
    def __init__(self):
        self.bucket_name = 'courseme-chatbot.appspot.com'
        self.blob_name = 'course_data_cache.pkl'
    ...
    def load_data(self):
        ...
        self.logger.info(f"Attempting to load cache from GCS bucket: {self.bucket_name}")

        if not blob.exists():
            self.logger.error(f"Cache file not found in GCS: {self.blob_name}")
            raise FileNotFoundError(f"Cache file not found in GCS: {self.blob_name}")

        self.logger.info(f"Cache file found in GCS. Size: {blob.size} bytes")

        if blob.size == 0:
            self.logger.error(f"Cache file in GCS is empty: {self.blob_name}")
            raise ValueError(f"Cache file in GCS is empty: {self.blob_name}")

        try:
            data = blob.download_as_bytes()
            self.course_data = pickle.loads(data)
            self.logger.info(f"Data loaded from GCS cache. Total courses: {len(self.course_data)}")
        except Exception as e:
            self.logger.error(f"Error loading cache from GCS: {str(e)}")
            raise ValueError(f"Error loading cache from GCS: {str(e)}")
